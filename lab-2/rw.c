...
void * writer_thr(void * arg) {
  ...
  for (j = 0; j < WRITE_ITR;j++) {
      found = FALSE;
      // now update
      pthread_mutex_lock(&rw_lock);  // writer locks readers and other writers out of critical section
      for (i = 0; i < SIZE; i++) {
        ...
        if (account_list[i].accno == update_acc[j].accno) {
          ...
          rest(); // rest to prevent a race condition before invalidating the account number

          account_list[i].accno = temp_accno;  // restore account number once balance is updated
...
void * reader_thr(void *arg) {
  ...
  */
  ...
  for (j = 0; j < READ_ITR; j++) {
    ...
    if (++read_count == 1) pthread_mutex_lock(&rw_lock);  // increase number of readers, keep writers from entering the rear/write critical section
    pthread_mutex_unlock(&r_lock);  // finally, unlock read lock

    for (i = 0; i < SIZE; i++) {
      rest(); // rest before locking the r mutex to avoid a race condition

      // now lock and update - remember that since account_list[i].accno can be updated by a writer, it MUST be protected by locking for ANY read!
